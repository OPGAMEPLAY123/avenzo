...
function loginWithGoogle() {
  ...
  _auth.signInWithPopup(provider).then(function(result) {
    var u = result.user;
    var users = S.users();
    var existing = Object.values(users).find(function(x) { return x.email === u.email; });
    if (!existing) {
      var id = 'u_' + Date.now();
      var code = 'AVNZ' + Math.floor(Math.random() * 100000);
      existing = { id: id, name: u.displayName || 'AvenZo User', username: (u.displayName || 'user').split(' ')[0].toLowerCase(), email: u.email, phone: '', coins: 50, referralCode: code, joinDate: new Date().toLocaleDateString(), avatar: u.photoURL || '' };
      users[id] = existing;
      S.saveU(users);
    }
    S.saveCur(existing);
